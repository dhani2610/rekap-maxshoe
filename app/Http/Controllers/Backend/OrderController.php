...
class OrderController extends Controller
{
    ...
    public function getDataKomisi(Request $request)
    {
        ...
        foreach ($summary['karyawan'] as $id => $data) {
            if (!isset($karyawanList[$id])) continue;
            ...
            switch ($data['posisi']) {
                case 'HOST':
                    ...
                    break;
                case 'CO HOST':
                    $orderIdsQuery = Order::where('co_host_id', $data['id']);
                    $komisiField = 'komisi_co_host';
                    break;
                case 'CS':
                    $orderIdsQuery = Order::where('cs_id', $data['id']);
                    $komisiField = 'komisi_cs';
                    break;
            }

            if ($orderIdsQuery) {
                if ($request->start_date && $request->end_date) {
                    $orderIdsQuery->whereBetween('created_at', [
                        $request->start_date . ' 00:00:00',
                        $request->end_date . ' 23:59:59'
                    ]);
                }
                ...
                $getItemOrder = OrderDetail::whereIn('order_id', $orderIds)->sum('jumlah');
                $getOmzetOrder = OrderDetail::whereIn('order_id', $orderIds)->sum('harga');
                $getKomisiOrder = Order::whereIn('id', $orderIds)->sum($komisiField);
            } else {
                $getItemOrder = 0;
                $getOmzetOrder = 0;
                $getKomisiOrder = 0;
        ...
        // Format angka
        $employee_card = array_map(function ($r) {
