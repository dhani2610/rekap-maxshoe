...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap" style="margin-top: 4px; margin-bottom: -30px;">
        ...
        <button class="filter-button2" id="downloadToSheet">
    ...
    <div class="mt-5">
        ...
        <script>
            ...
            const API_KEY = "{{ env('GOOGLE_DEVELOPER_KEY') }}";
            const SPREADSHEET_ID = "{{ env('POST_SPREADSHEET_ID') }}";
            const SHEET_NAME = "{{ env('POST_SHEET_NAME') }}";
            ...
            function gisLoaded() {
                tokenClient = google.accounts.oauth2.initTokenClient({
                    client_id: CLIENT_ID,
                    scope: SCOPES,
                    callback: '', // nanti diisi setelah signIn
                });
                gisInited = true;
            }

            // Convert Table HTML ke Array
            ...
            // Upload Data
            $("#downloadToSheet").on("click", function() {
                if (!gapiInited || !gisInited) {
                    alert("Google API belum siap.");
                    return;
                }
                ...
                tokenClient.callback = async (resp) => {
                    ...
                    let response = await fetch("{{ route('data-google-sheets') }}");
